...
public final class Matcher extends AbstractJobLauncher {
	...
	// public static final String CONTAINS_DEL = CONTAINS_PROPERTY +
	// CGEntity.SUB_DELIMITER;
	// public static final byte[] CONTAINS_DEL_BYTES =
	// CONTAINS_DEL.getBytes(Charsets.UTF_8);

	public static void main(final String[] args) {
		launch(new Matcher(), args);
	}
	...
	static final class UniquePropertiesReducer extends TableReducer<Text, Text, Text> {
		...
		private final Set<String> movetoNotices = new HashSet<String>();
		private final Set<String> containsIdList = new HashSet<String>();
		...
		public void reduce(final Text key, final Iterable<Text> values, final Context context) throws IOException,
			...
			for (Text val : values) {
				if (val.find(EQUAL_DEL) == 0) {
					movetoNotices.add(val.toString());
				} else {
					containsIdList.add(val.toString());
				}
			}
			...
			movetoNotices.remove(EQUAL_DEL + key);
		...
		private void writeBundle(final Text bundleId, final Context context) throws IOException, InterruptedException {
			...
			for (String containsId : containsIdList) {
				putWriter.literal(CONTAINS_PROPERTY, containsId);
			}
			for (String moved : movetoNotices) {
				putWriter.literal(moved.getBytes());
			}
			putWriter.endRecord();
